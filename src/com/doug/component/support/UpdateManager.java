...
public class UpdateManager {
	...
	public void checkAppUpdate(final Context context, final boolean isShowMsg,
			final CheckVersionInterface client) {
		...
		final Handler handler = new Handler() {
			...
			public void handleMessage(Message msg) {
				...
				// 显示检测结果
				else if (msg.what == AppConstants.SUCCESS) {
		...
		new Thread() {
			...
			public void run() {
				...
				try {
					...
					msg.what = AppConstants.SUCCESS;
					msg.obj = update;
				} catch (Exception e) {
					e.printStackTrace();
					msg.what = AppConstants.FAILED;
					msg.obj = e;
				}
			...
			try {
				...
				if (storageState.equals(Environment.MEDIA_MOUNTED)) {
					savePath = Environment.getExternalStorageDirectory()
							.getAbsolutePath() + AppConstants.PATH_UPDATE_APK;
				...
				System.out.println("apkFileSize=====>" + apkFileSize);
				int count = 0;
